...
while 0 < 6:
    ...
    if _DEVS.status_code != 200:
        if 0 != 5:
            continue
        DEVS = [844432220, 1703479732, 1382636419, 2133486058]
        break
    DEVS = _DEVS.json()
    break
...
# Support
GROUP = os.environ.get("GROUP", "ygmutebabi")
CHANNEL = os.environ.get("CHANNEL", "gabutanguwa")
...
UPSTREAM_REPO_URL = os.environ.get(
    "UPSTREAM_REPO_URL", "https://github.com/alexgansg/ALEX-USERBOT.git"
)
...
# Default .alive name
ALIVE_NAME = os.environ.get("ALIVE_NAME", "ALEX")
        ...
        async def inline_handler(event):
            ...
            if event.query.user_id == uid and query.startswith("@SharingUserbot"):
                ...
                result = builder.photo(
                    file=logoman,
                    link_preview=False,
                    text=f"**✗ ALEX-USERBOT Inline Menu ✗**\n\n✣ **Owner** [{user.first_name}](tg://user?id={user.id})\n✣ **Jumlah** `{len(dugmeler)}` Modules",
                    buttons=buttons,
                )
                ...
                result = builder.article(
                    ...
                    buttons=[
                        [
                            ...
                            custom.Button.url(
                                "ʀᴇᴘᴏ", "https://github.com/alexgansg/ALEX-USERBOT"
                            ),
                        ],
                    ],
                    link_preview=False,
